...
import {
    ...
    empresaError,
} from '../../../state/actions/empresa.actions';
...
export class UpEmpresaComponent {
    ...
    ngOnInit() {
        ...
        this.subscription.add(
            this.store.select(selectLogoEmpresa).subscribe((res: any) => {
                if (res.length > 0) {
    ...
    public guardar(
        ...
        if (this.up.valid) {
            ...
            this.actions$.pipe(ofType(updateEmpresaSuccess)).subscribe(() => {
                setTimeout(() => {
                    this.val = false;
                    this.router.navigate([
                        '/desk/seguridad/administracion/empresa',
                    ]);
                }, 1000);
            });

            // Suscribirse a la acción de error
            this.actions$.pipe(ofType(empresaError)).subscribe((error) => {
                this.val = false;

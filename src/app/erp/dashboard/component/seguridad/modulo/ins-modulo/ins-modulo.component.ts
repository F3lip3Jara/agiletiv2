...
export class InsModuloComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.actions$
            .pipe(ofType(getOpcionesNoAsignadasModuloByIdSuccess))
            .subscribe((response) => {
        ...
        this.actions$
            .pipe(ofType(getRolesNoAsignadasModuloByIdSuccess))
            .subscribe((response) => {
                this.sourceRoles = [...response.roles];
            });

        // Manejar errores
        this.actions$.pipe(ofType(moduloError)).subscribe((error) => {
    ...
    guardarModulo(molIcon: string, molDes: string, opt: any[], rol: any[]) {
        ...
        this.actions$
            .pipe(ofType(createModuloSuccess))
            .subscribe((response) => {
                setTimeout(() => {
                    this.loading = false;
                    this.volver();

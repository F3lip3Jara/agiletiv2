import { Component, OnInit, ViewChild, ElementRef, NgZone, LOCALE_ID, OnDestroy } from '@angular/core';
...
import { getEmpresaPdfRequest, getEmpresaPdfSuccess, getPedidoProductosRequest, getPedidoProductosSuccess, updatePedidonacRequest, updatePedidonacSuccess } from '../../state/actions/pedidonac.actions';
...
@Component({
  selector: 'app-up-pedido',
  templateUrl: './up-pedido.component.html',
  styleUrl: './up-pedido.component.scss'
})
export class UpPedidoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Luego obtenemos los parámetros de la ruta
    this.route.params.pipe(
      take(1)
    ).subscribe(params => {
      this.pedido = JSON.parse(atob(params['pedido']));             
      // Solo actualizamos el formulario si ya está inicializado
      if (this.pedidoForm) {
    ...
    // Después despachamos las acciones iniciales
    ...
    // Suscripción para los productos del pedido
    this.subscription.add(
      this.actions$.pipe(
  ...
  private initForm() {
    ...
    // Suscribirse a cambios en el proveedor
    this.subscription.add(
  ...
  filtrarAlmacenes(centroId: number) {      
    ...
    this.actions$.pipe(
      ...
      this.almacenesFiltrados = this.almacenes.filter(alm => alm.centroId === centroI);
      // Si hay un pedido original y tiene un almacén, seleccionarlo
      if (this.pedido && this.pedido.almacen_id) {
        const almacenSeleccionado = this.almacenesFiltrados.find(alm => alm.almId === this.pedido.almacen_id);
        if (almacenSeleccionado) {
          this.pedidoForm.patchValue({
            almacen: almacenSeleccionado.almId
          });
        }
      }
    });
  ...
  descargarPDF() {
    try {
      ...
      // Agregar logo de la empresa si existe
      ...
      // Información de fecha y número de orden (esquina superior derecha)
      doc.setFontSize(10);
      ...
      // Información del remitente (esquina superior izquierda)
      doc.setFontSize(10);
  ...
  filtrarProductos() {
    ...
    const busqueda = this.busquedaSKU.toLowerCase();
    this.productosFiltrados = this.productos.filter(producto => 
      producto.cod_pareo?.toLowerCase().includes(busqueda) || 
  ...
  actualizarPedido(){
    if (this.pedidoForm.valid) {
      ...
      this.store.dispatch(updatePedidonacRequest({ pedidonac: pedido }));
       this.actions$.pipe(
        ofType(updatePedidonacSuccess),
        take(1)
       ).subscribe(() => {
        setTimeout(() => {
          this.loading = false;
